...
const ChatHome = () => {
  ...
  const followingUsers = Array.isArray(users) ? users.filter((u) => Array.isArray(user?.following) && user.following.map(f => f.toString()).includes(u._id)) : []

  const filteredUsers = followingUsers.filter((u) => u.fullName.toLowerCase().includes(searchQuery.toLowerCase()))
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-4rem)] max-w-md  p-4">
                ...
                {filteredUsers.map((chatUser) => (
                  ...
                  >
                    <Avatar className="h-12 w-12">
                      <AvatarImage src={chatUser.image || ""} alt={chatUser.fullName} />
                      <AvatarFallback>{getInitials(chatUser.fullName)}</AvatarFallback>
                    </Avatar>
